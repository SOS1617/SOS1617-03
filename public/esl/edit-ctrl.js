angular
    ...
    .controller("EslEditCtrl",["$scope","$http","$routeParams","$location",function ($scope, $http, $routeParams, $location){
        ...
        function refresh(){
            $http
            .get("/api/v1/earlyleavers/"+$routeParams.country + "/" + $routeParams.year+ "?" + "apikey=" + $scope.apikey)
        ...
        $http
            .put("../api/v2/earlyleavers/" + data.country + "/" + data.year + "?" + "apikey=" + $scope.apikey, data)
            .then(function(response) {
